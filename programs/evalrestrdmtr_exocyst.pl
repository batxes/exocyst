...
open (CHMY2H,">chimeraEvaldmtrY2H_exocyst.py");
...
$interaction_ok = 0;
$j= 0;
    open (CHI_Y2H,"chimera --nogui chimeraEvaldmtrY2H_exocyst.py |");
...
sub Add_all_beads_y2h(){
    ...
    if ($residue =~ /Sec3/){
        ...
        if ($residue =~ /Sec5/ || $residue =~ /Sec6/ || $residue =~ /Sec8/ || $residue =~ /Sec10/ || $residue =~ /Sec15/){
            $beads = 6;
        } else {
            if ($residue =~/Exo70/){
                $beads = 5;
