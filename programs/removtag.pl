...
if (/mark=s\.place_marker/) {
 ...
 #$prt=1 if (/if \"\w+_\d+\" not in marker_sets:/ && (/Cog1/ || /Cog2/ || /Cog3/ || /Cog4/ || /Cog5/ || /Cog6/ || /Cog7/ || /Cog8/) || /Sec2_GFPC/);
# $prt=0 if (/if \"\w+_\w+\" not in marker_sets:/);
# $prt=1 if (/if \"Exo84_\w+\" not in marker_sets:/);
