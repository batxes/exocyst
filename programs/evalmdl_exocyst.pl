...
open (DSP,"./input/displ_exocyst_Tags.xml");
#open (DSP,"./input/displ_sec2.xml");
$id=-1;
while (<DSP>) {next if (/--/);
	if (/<Chain><Fragment id="(\S+)"><Color/) {$id++;$id{$1}=$id;$di{$id}=$1;}
}

#open (RES,"./input/restraint_TagsOnly.xml");
open (RES,"./input/restraint_exocyst_Tags.xml");
#open (RES,"./input/Distances_NoDuplicates_restraint.xml");
while (<RES>) {
 last if (/<!--/);
last if (/Diameter/);
